...
#    tf.set_random_seed(1234)
    ...
    with tf.Graph().as_default(), tf.device('/device:gpu:0'):
        ...
        num_batches_per_epoch = int(0.5 * dataset_size  / cfg.batch_size )
        ...
        for dev in ['/device:gpu:0','/device:gpu:1','/device:gpu:2','/device:gpu:3']:
            with tf.device(dev):
                ...
                output, pose_out,_ = net.build_arch(batch_x, coord_add, is_train=True, num_classes=num_classes)
                    ...
                    try:
                        ...
                        if ((step % num_batches_per_epoch*10)) == 0:
                            ...
                            ckpt_path = os.path.join(
                                cfg.logdir + '/caps/{}/'.format(dataset_name), 'model.ckpt')
